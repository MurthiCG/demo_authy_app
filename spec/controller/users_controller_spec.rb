...
RSpec.describe Api::UsersController, type: :controller do
  ...
  describe "POST sign_in" do
    ...
    context "Not confirm user" do
      ...
      it "with authy enabled" do
        ...
        post :sign_in, params: params
        expect(response.status).to eq(404)
        expect(json(response.body)).to eq(json({success: false, msg: 'User not Active. please click the confirm button.'}.to_json))
      end  
    end   
  ...
  describe "PUT change_password" do
    ...
    it "invalid current password" do
      ...
      expect(json(response.body)).to eq(json({success: false, message: "Invalid Password"}.to_json))
    end

    it "password and current password are same" do
      request.headers.merge!({'Authorization' => token})
      put :change_password, params: {id: user.id, user: {current_password: "Password@123", password: "Password@123"}}
      expect(json(response.body)).to eq(json({success: false, message: "New password cannot be the same as the current password."}.to_json))
    end  

    it "password and confirm password doesn't match" do
      request.headers.merge!({'Authorization' => token})
      put :change_password, params: {id: user.id, user: {current_password: "Password@123", password: "Password@1234", confirm_password: "Password@12345"}}
    ...
    it "password length and alphanueric check" do
      request.headers.merge!({'Authorization' => token})
      put :change_password, params: {id: user.id, user: {current_password: "Password@123", password: "Pass", confirm_password: "Pass"}}
      expect(json(response.body)["password"].count).to eq(2)
    end

    it "successful update" do
      request.headers.merge!({'Authorization' => token})
      put :change_password, params: {id: user.id, user: {current_password: "Password@123", password: "Password@1234", confirm_password: "Password@1234"}}
